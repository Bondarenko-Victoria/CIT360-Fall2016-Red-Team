...
public class HibernateRun {
    ...
    public static void addStudent(EntityManagerFactory myManagerFactory, Scanner input){
        ...
        // NASTY PATH: Without the split, it will take it as one long string for the first name
        // causing the last name to be empty, and won't compile because the insert value for last name
        // would be empty.
...
// NASTY PATHS:
/* Misspelling the persistence unit in the manager factory to pass as parameter, it will throw errors such as
 * that it cannot find a persistence xml file.
 * Putting the switch statement outside the while loop, it will create an infinite loop.
 * Not having foreign keys will cause a couple of compilation errors. The only methods that would still
 * work are the adding of a new student and the adding of a new phone number.
 * Having the same column name in multiple tables in the actual database can confuse you, and it would be hard to debug your code.
 * Having the same column name in multiple java files that represent different tables will also confuse you and cause mapping errors.
 * Not having the "updatable=false" and "insertable=false" would create run time errors when you try to add a phone to a student.
 * If you want to auto increment the primary keys, it must be reflected in both the database field and in field in the java file.
 * Putting the annotations in the wrong place and throw lots of errors. The right way is to move the declaration of the field
 * by itself and putting the annotations above it, and your getters and setters should be below the declared field.
 * Setting the id's to the right id's to patch with the other table in the setters can throw foreign key constraint exception errors.
 * Leaving the username and password empty in the persistence xml file will cause connection errors.
 * The hardest parts of the set up was being able to connect add a connection to my database and connect to it.
 * Also downloading and adding the correct tomcat catalina and mysql java connector libraries. I tried more than 10 times
 * to create a new project, because I could not connecto to my database, or because the libraries were not compatible
 * with the persistence xml file.
 * The hardest parts of running my code without errors were having the correct relationships within my tables in my
 * database, and also having the right annotations so that they would match my database. Also, having the same declared fields
 * with their correct getters and setters.
 */